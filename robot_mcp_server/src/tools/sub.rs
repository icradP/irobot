...
#[schemars(description = "Difference of two numbers")]
pub struct SubRequest {
    #[schemars(description = "First number (minuend)")]
    pub a: i32,
    #[schemars(description = "Second number (subtrahend)")]
    pub b: i32,
}

impl rmcp::service::ElicitationSafe for SubRequest {}

#[derive(Debug, Serialize, Deserialize, JsonSchema)]
#[schemars(description = "Sub 引导参数")]
pub struct SubElicitation {
...
impl rmcp::service::ElicitationSafe for SubElicitation {}

pub fn tool() -> ToolEntry {
    let schema = schemars::schema_for!(SubRequest);
    let tool = Tool {
        name: "sub".into(),
        title: Some("Subtraction".into()),
        description: Some("[Utility] Calculate the difference of two numbers".into()),
...
pub async fn handle(
    ...
    if let Some(args) = request {
        ...
        if let Ok(parsed) = serde_json::from_value::<SubRequest>(args.clone()) {
            a = Some(parsed.a);
            b = Some(parsed.b);
        } else {
    ...
    let mut prompt = "请提供两个数字用于减法计算".to_string();
    for _ in 0..max_attempts {
        ...
        let elicit_result = tokio::select! {
            ...
            r = context.peer.elicit::<SubElicitation>(prompt.clone()) => r,
        };
        ...
        match elicit_result {
            Ok(Some(params)) => {
                a = params.a;
                b = params.b;
                prompt = "请提供两个数字用于减法计算".to_string();
            }
            Ok(None) => {
