...
pub async fn handle(
    ...
    let args: GetWeatherRequest = if let Some(args) = request {
        ...
        match context.peer.elicit::<GetWeatherRequest>("请提供城市名称（例如：北京、上海、杭州）".to_string()).await {
    ...
    let mut url = Url::parse("http://api.weatherapi.com/v1/current.json").map_err(|e| ErrorData::internal_error(e.to_string(), None))?;
    url.query_pairs_mut()
    ...
    if !status.is_success() {
        return Err(ErrorData::internal_error(format!("Weather API error: {}", raw), None));
    }
    ...
    let text = format!(
        ...
        location, region, country, condition, 
        temp_c, feelslike_c, 
        wind_dir, wind_kph, 
        humidity, uv, aqi_msg
    );
